-- landing_stats: agrega contadores agregados de censo (sin PII) junto a necesidades de insumos.
...
returns json language sql security definer set search_path = public stable as $$
  ...
  select json_build_object(
    'total_patients', (select count(*)::int from patients),
    'registered_today', (
      select count(*)::int from patients where created_at::date = now()::date
    ),
    'collection_centers', (
