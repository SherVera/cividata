...
export default function PatientForm({ initialPatient, onSave, onCancel }: PatientFormProps) {
  ...
  useEffect(() => {
    if (!requiereRepresentante && optionalSections.representante) {
      setOptionalSections((prev) => ({ ...prev, representante: false }));
    }
  }, [requiereRepresentante, optionalSections.representante]);
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    if (firstInvalid === 0) {
      ...
      const tabName = formTabs.find((t) => t.id === firstInvalid)?.label || `Paso ${firstInvalid}`;
  ...
  const formTabs = [
    ...
    { id: 3, label: 'Datos adicionales', icon: Heart },
  ];
